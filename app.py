...
# Se il path nel CSV non funziona, cerca qui
IMAGES_FOLDER = 'train' 
...
COURT_WIDTH = 3840 
COURT_HEIGHT = 2160
POSSESSION_THRESHOLD = 60 # Pixel (Box-in-Box tolerance)

# Parametri Fisici
FPS = 30.0
REAL_WIDTH_M = 28.0
REAL_HEIGHT_M = 15.0
# Fattore di Conversione: 1 Pixel = ~0.007 metri
PX_TO_M = REAL_WIDTH_M / COURT_WIDTH 

# Filtri Fisici
MAX_PIXEL_STEP = 100 # Ignora salti > 70cm per frame
SMOOTHING_WINDOW = 5
...
def load_data():
    ...
    # Creiamo colonne metri AL VOLO (così siamo sicuri della conversione)
...
def get_possession_table(df_subset):
    ...
    # Conflict Res: Vince chi è più vicino in PIXEL
...
# --- CALCOLO STATISTICHE IBRIDE (Pixel Logic -> Output Metri) ---
def calculate_advanced_stats_hybrid(df_action, player_id, current_frame, ownership_table):
    ...
    # 2. Velocità (km/h) su ultimi 15 frame
    speed_kmh = 0.0
    if len(p_data) > 15:
    ...
    # 3. Possesso (Tabella Globale)
    p_data = p_data.join(ownership_table, on='frame_id', rsuffix='_owner')
    p_data['is_mine'] = (p_data['player_unique_id_owner'] == player_id)
    
    # Buffer (3 frame)
    p_data['is_mine_buffered'] = p_data['is_mine'].rolling(window=3, center=True, min_periods=1).sum() >= 2
    
    off_ball_px = step_px[~p_data['is_mine_buffered']].sum()
    off_ball_m = off_ball_px * PX_TO_M
    poss_frames = p_data['is_mine_buffered'].sum()
...
# --- RENDERING VIDEO (Ottimizzato per Streaming) ---
def render_dual_view(f_id, df, quality_mode, highlight_id=None, is_possessor=False):
    ...
    # RESIZE AGGRESSIVO PER FLUIDITÀ (Max 1000px larghezza in playback)
    ...
    sx, sy = 600 / REAL_WIDTH_M, RADAR_HEIGHT / REAL_HEIGHT_M # Radar: Metri -> Pixel
    ...
    for _, row in frame_data.iterrows():
        # Radar usa METRI (calcolati al volo da pixel)
        ...
        if t_str == 'Ball':
            ...
            if highlight_id and row['player_unique_id'] == highlight_id:
                ...
                # Check Box Overlap Pixel (Visual)
...
def update_ui_elements(fid, dist_tot, dist_off, poss_frames, speed):
    ...
    # OTTIMIZZAZIONE MEMORIA: usa buffer JPEG per video fluido
    success, buffer = cv2.imencode(".jpg", vid, [int(cv2.IMWRITE_JPEG_QUALITY), 50])
    vid_bytes = buffer.tobytes()
    
    if vid is not None:
        video_ph.image(vid_bytes, channels="RGB", use_container_width=True)
...
if analysis_mode == "🕹️ Navigazione (Manuale)":
    ...
    if st.sidebar.button("▶️ PLAY", type="primary"):
        ...
        for f_id in frames:
            ...
            time.sleep(max(0.03, (1.0/fps) - (time.time()-t0))) # Throttle aumentato a 30ms
    ...
    if c1.button("Genera Metriche"):
        with st.spinner("Calcolo..."):
            ...
            if moves:
                ...
                # Fix NaN in speed/poss
                sp_red = sp_df[sp_df['Team']=='Red']
                sp_white = sp_df[sp_df['Team']=='White']
                
                asp_r = sp_red['Speed'].mean() if not sp_red.empty else 0
                ap_r = sp_red['Poss'].mean() if not sp_red.empty else 0
                
                aw_t = mdf[(mdf['Team']=='White')&(mdf['Type']=='Total')]['Dist'].mean()
                aw_o = mdf[(mdf['Team']=='White')&(mdf['Type']=='Off-Ball')]['Dist'].mean()
                asp_w = sp_white['Speed'].mean() if not sp_white.empty else 0
                ap_w = sp_white['Poss'].mean() if not sp_white.empty else 0
