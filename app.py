...
IMAGES_FOLDER = 'datasets' # Cerca qui le immagini
RADAR_HEIGHT = 300
COURT_WIDTH = 3840 
COURT_HEIGHT = 2160
...
def render_dual_view(f_id, df, quality_mode, highlight_id=None, is_possessor=False):
    ...
    if frame_img_orig is None: 
        # Fallback: immagine nera con testo
        frame_img_orig = np.zeros((720, 1280, 3), dtype=np.uint8)
        cv2.putText(frame_img_orig, "IMG ERROR", (50, 360), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)

    # Ridimensiona a larghezza contenuta (800px) per evitare lag browser
...
def update_ui_elements(fid, dist_tot, dist_off, poss_frames, speed):
    ...
    # COMPRESSIONE JPEG (FIX PER ERRORI 'MISSING FILE' e FLUIDITA')
    if vid is not None:
        ...
        for f_id in frames:
            ...
            if not curr.empty:
                ...
                if len(pos_buffer)>=2:
                    ...
                    raw_px = np.sqrt((p_now[0]-p_prev[0])**2 + (p_now[1]-p_prev[1])**2)
                    if raw_px < MAX_PIXEL_STEP:
                        step_m = raw_px * PX_TO_M
                        cum_m += step_m
            ...
            # SLOW DOWN FOR BROWSER (Fondamentale per Cloud)
    ...
    if c1.button("Genera Metriche"):
        with st.spinner("Calcolo..."):
            ...
            if moves:
                ...
                k1, k2 = st.columns(2)
                k1.info(f"🔴 Red: Tot {ar_t:.1f}m | Off {ar_o:.1f}m | Spd {asr:.1f}km/h | Poss {apr:.1f}s")
    ...
    if c3.button("Heatmap Azione"):
        ...
        for t in ['Red', 'White']:
            ...
            if not st_t.empty:
                ...
                ax.set_xlim(0, COURT_WIDTH); ax.set_ylim(COURT_HEIGHT, 0); ax.axis('off'); ax.set_title(f"Heatmap {t}")
                c3.pyplot(fig)