...
IMAGES_FOLDER = 'datasets' # Fallback
RADAR_HEIGHT = 300
COURT_WIDTH = 3840 
COURT_HEIGHT = 2160
...
REAL_WIDTH_M = 28.0
REAL_HEIGHT_M = 15.0
# Fattore: 1 metro = ~137 pixel
PX_TO_M = REAL_WIDTH_M / COURT_WIDTH 
# Filtro Anti-Teletrasporto: > 100px/frame (circa 0.7m) è errore
MAX_PIXEL_STEP = 100 
# Smoothing
SMOOTHING_WINDOW = 5
...
def draw_mpl_court(ax, color='black', lw=2):
    # Disegna in PIXEL per coerenza con heatmap
...
# --- CALCOLO STATISTICHE IBRIDE (Logic Pixel -> Out Metri) ---
def calculate_advanced_stats_hybrid(df_action, player_id, current_frame, ownership_table):
    ...
    if len(p_data) > 15:
        ...
        if speed_kmh < 1.0: speed_kmh = 0.0 # Noise gate
...
def render_dual_view(f_id, df, quality_mode, highlight_id=None, is_possessor=False):
    ...
    sx, sy = 600 / REAL_WIDTH_M, RADAR_HEIGHT / REAL_HEIGHT_M # Radar in Metri
    red_c, white_c, ref_c = 0, 0, 0
    ball_row = frame_data[frame_data['team'] == 'Ball']

    for _, row in frame_data.iterrows():
        # Radar
        xm = row['x_meters']; ym = row['y_meters']
...
def update_ui_elements(fid, dist_tot, dist_off, poss_frames, speed):
    ...
    if vid is not None:
        ...
        radar_ph.image(rad, channels="RGB", caption="Tactical", use_container_width=True)
        ...
        for f_id in frames:
            ...
            if not curr.empty:
                ...
                if len(pos_buffer)>=10:
                    ...
                    if raw_spd > 3.0: speed_kmh = min(raw_spd, 36.0)
    ...
    if c1.button("Genera Metriche"):
        with st.spinner("Calcolo..."):
            ...
            if moves:
                ...
                # --- FIX: Definisco colonne grafici qui dentro ---
                chart1, chart2, chart3 = st.columns(3)
                
                # Info in alto (o in chart1)
                st.info(f"🔴 Red: Tot {atr:.1f}m | Off {aro:.1f}m | Spd {asr:.1f}km/h | Poss {apr:.1f}s")
                st.info(f"⚪ White: Tot {awt:.1f}m | Off {awo:.1f}m | Spd {asw:.1f}km/h | Poss {apw:.1f}s")

                fig2, ax2 = plt.subplots(figsize=(6, 5))
                sns.barplot(data=mdf, x='Player', y='Dist', hue='Type', palette={'Total':'gray', 'Off-Ball':'limegreen'}, ax=ax2)
                ax2.axhline(atr, c='darkred', ls='--'); ax2.axhline(aro, c='red', ls=':')
                ax2.axhline(awt, c='darkblue', ls='--'); ax2.axhline(awo, c='blue', ls=':')
                ax2.tick_params(axis='x', rotation=90); ax2.set_title("Workload (m)"); chart1.pyplot(fig2)
                
                fig3, ax3 = plt.subplots(figsize=(6, 4))
                sns.barplot(data=spdf, x='Player', y='Poss', hue='Team', palette={'Red':'red', 'White':'blue'}, ax=ax3)
                ax3.tick_params(axis='x', rotation=90); ax3.set_title("Possession (s)"); chart2.pyplot(fig3)
                ...
                ax4.tick_params(axis='x', rotation=90); ax4.set_title("Avg Speed (km/h)"); chart3.pyplot(fig4)
